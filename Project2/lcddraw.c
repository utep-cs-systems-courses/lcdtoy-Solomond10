...
void drawChar7x9(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  ...
  lcd_setArea(rcol, rrow, rcol + 6, rrow + 9); /* relative to requested col/row */
  while (row < 10) {
    while (col < 7) {
      u_int colorBGR = (font_7x9[oc][col] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      col++;
    }
    col = 0;
 ...
 */
void drawString7x9(u_char col, u_char row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
    drawChar7x9(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 8;
  }
}
...
/** 4x8 font - this function draws background pixels
 *  Adapted from RobG's EduKit
 */
void drawChar4x8(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
  ...
  lcd_setArea(rcol, rrow, rcol + 3, rrow + 8); /* relative to requested col/row */
  while (row < 9) {
    while (col < 4) {
      u_int colorBGR = (font_4x8[oc][col] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      col++;
    }
    col = 0;
 ...
 *  \param bgColorBGR Background color in BGR
 */
void drawString4x8(u_char col, u_char row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
    drawChar4x8(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 5;
  }
}
